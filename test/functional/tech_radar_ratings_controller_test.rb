...
class TechRadarRatingsControllerTest < Redmine::ControllerTest
  ...
  def test_show_renders_technology_and_prefilled_rating
    TechRadar::Rating.create!(user: @admin, technology: @t1,
                              can_level: :advanced, want_level: :yes)
    ...
    assert_select '.tech-radar-card[data-rating-card-previous-can-level-value=?]', 'advanced'
  end
  ...
  def test_show_renders_skip_link_to_next_unrated
    get :show, params: { technology_id: @t1.id }

    assert_response :success
    assert_select "a[data-rating-card-target='skip'][href=?]", tech_radar_rate_technology_path(@t2)
  end

  def test_show_hides_skip_link_on_last_unrated
    TechRadar::Rating.create!(user: @admin, technology: @t1,
                              can_level: :advanced, want_level: :yes)
    ...
    assert_response :success
    assert_select "a[data-rating-card-target='skip']", count: 0
